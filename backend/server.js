...
app.post('/api/empleados/crear-password', async (req, res) => {
  try {
    ...
    if (empleados.length === 0) {
      return res.status(404).json({
        mensaje: 'No existe un empleado registrado con ese correo.'
      });
    }
    ...
    await db.promise().query(
      ...
      WHERE IdEmpleado = ?
      `,
      [contrasena, empleados[0].IdEmpleado]
    );
    ...
    return res.status(500).json({
      mensaje: 'No se pudo guardar la contraseña.'
    });
  }
});
require('./routes/portalEmpleado')({
  app,
  dbPromise,
  jwt,
  bcrypt,
